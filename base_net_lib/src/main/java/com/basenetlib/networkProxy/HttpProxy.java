...
import com.basenetlib.loadingDialog.LoadingDialog;
...
public class HttpProxy<H extends HttpProxy> {
    ...
    private boolean  showLoadingDialog = false;//展示加载动画  默认显示
     ...
     * @return
     */
    public H loadingDialogShow(boolean value) {

        showLoadingDialog = value;
        return (H) this;
    }
    ...
    public void postToNetwork(String urlPath, NetResponseCallBack netCallBackInterface) {
        if (NetWorkUtil.isNetworkAvailable()) {
            if (showLoadingDialog) {
                LoadingDialog.getInstance().showProgress(NetWorkUtil.context);
            }
